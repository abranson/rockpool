...
class RockworkNotificationSourcesTest {
    @Test
    fun `mapping preserves conditional mode and hard mute`() {
        ...
        assertEquals(
            listOf(
                RockworkNotificationSource("conditional", "Conditional", "", 1, null, null),
                RockworkNotificationSource("muted", "Muted", "", 0, null, null),
            ),
    ...
    fun `tracker emits additions changes and one removal`() {
        val tracker = RockworkNotificationSourceTracker()
        val first = RockworkNotificationSource("one", "One", "", 2, null, null)
        val second = RockworkNotificationSource("two", "Two", "", 1, null, null)
        ...
        assertEquals(
            listOf(RockworkNotificationSource("two", "", "", -1, null, null)),
    ...
    fun `publisher emits old then newest snapshot when its first emission is held`() = runBlocking {
        val old = RockworkNotificationSource("mail", "Mail", "", 2, null, null)
        val newest = old.copy(enabled = 0)
        var snapshot = listOf(old)
        val emitted = mutableListOf<RockworkNotificationSource>()
        ...
        val publisher = RockworkNotificationSourcePublisher(
            snapshot = { snapshot },
            emit = { source ->
