...
class RockpoolLanguagePackTest {
    @Test
    fun `legacy source parser distinguishes downloads and local files`() {
        val remote = assertIs<RockpoolLanguagePackSource.Remote>(
            parseRockpoolLanguagePackSource(
                "https://binaries.rebble.io/lp/OHRO2oz-it_IT.pbl",
            ),
        )
        assertEquals("OHRO2oz-it_IT", remote.name)

        val fileUrl = assertIs<RockpoolLanguagePackSource.Local>(
            parseRockpoolLanguagePackSource("file:///tmp/Italian%20pack.pbl"),
        )
        assertEquals("/tmp/Italian pack.pbl", fileUrl.path.toString())
        assertEquals("Italian pack", fileUrl.name)

        val local = assertIs<RockpoolLanguagePackSource.Local>(
            parseRockpoolLanguagePackSource("/tmp/de_DE.pbl"),
        )
        assertEquals("de_DE", local.name)

        val rawSpace = assertIs<RockpoolLanguagePackSource.Local>(
            parseRockpoolLanguagePackSource("/tmp/Italian pack.pbl"),
        )
        assertEquals("/tmp/Italian pack.pbl", rawSpace.path.toString())

        val rawPercent = assertIs<RockpoolLanguagePackSource.Local>(
            parseRockpoolLanguagePackSource("file:///tmp/100% Italian.pbl"),
        )
        assertEquals("/tmp/100% Italian.pbl", rawPercent.path.toString())

        val relativeColon = assertIs<RockpoolLanguagePackSource.Local>(
            parseRockpoolLanguagePackSource("custom:Italian pack.pbl"),
        )
        assertEquals("custom:Italian pack.pbl", relativeColon.path.toString())

        assertFailsWith<IllegalArgumentException> {
            parseRockpoolLanguagePackSource("ftp://example.test/de_DE.pbl")
        }
    }
    ...
    fun `installer routes URLs and paths to the matching libpebble overload`() {
        ...
        installRockpoolLanguagePack(
            parseRockpoolLanguagePackSource("https://example.test/en_US.pbl"),
            installRemote = { url, name -> calls += "remote:$url:$name" },
            installLocal = { path, name -> calls += "local:$path:$name" },
        )
        installRockpoolLanguagePack(
            parseRockpoolLanguagePackSource("file:///tmp/fr_FR.pbl"),
            installRemote = { url, name -> calls += "remote:$url:$name" },
            installLocal = { path, name -> calls += "local:$path:$name" },
        )
    ...
    fun `language status uses the legacy locale version shape and completion edge`() {
        ...
        val initial = rockpoolLanguageStatus(base)
        ...
        val disconnected = rockpoolLanguageStatus(fakeWatch(connected = false))
