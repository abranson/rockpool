...
class RockpoolSignalContractTest {
    @Test
    fun `installed applications change has no payload`() {
        val signal = RockpoolPebble.InstalledAppsChanged(WATCH_PATH)
    ...
    fun `health data change retains legacy empty payload`() {
        val signal = RockpoolPebble.HealthDataChanged(WATCH_PATH)
    ...
    fun `health history methods retain legacy D-Bus shapes`() {
        val overview = RockpoolPebble::class.java.getMethod("HealthOverview")
        val fetch = RockpoolPebble::class.java.getMethod("FetchHealthData")
    ...
    fun `weather locations change retains legacy variant array payload`() {
        ...
        val signal = RockpoolPebble.WeatherLocationsChanged(WATCH_PATH, locations)
