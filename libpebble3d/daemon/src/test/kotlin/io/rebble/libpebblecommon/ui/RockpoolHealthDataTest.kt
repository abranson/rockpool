...
class RockpoolHealthDataTest {
        ...
        assertEquals(
            ...
            unwrap(
                coordinator.overlayDataForRange(
                    dayStart,
                    dayStart + ROCKPOOL_HEALTH_MAX_DETAIL_SECONDS,
                    OverlayType.Walk.value,
                ),
            ),
        )
    ...
    fun `invalid and oversized reads fail before querying health storage`() = runBlocking {
        ...
        assertFailsWith<IllegalArgumentException> {
            coordinator.stepsDataForRange(0, ROCKPOOL_HEALTH_MAX_DETAIL_SECONDS + 1)
        }
        assertFailsWith<IllegalArgumentException> {
    ...
    private fun coordinator(fake: HealthDataApi, date: LocalDate) = RockpoolHealthDataCoordinator(
        health = fake,
