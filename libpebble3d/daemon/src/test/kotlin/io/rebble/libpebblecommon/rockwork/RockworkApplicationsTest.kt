...
class RockworkApplicationsTest {
    @Test
    fun `account-global mutations are allowed only for the sole addressed watch`() {
        assertTrue(rockworkGlobalAppMutationAllowed("AA:BB", listOf("aa:bb")))
        assertFalse(rockworkGlobalAppMutationAllowed("AA:BB", emptyList()))
        assertFalse(rockworkGlobalAppMutationAllowed("AA:BB", listOf("CC:DD")))
        assertFalse(rockworkGlobalAppMutationAllowed("AA:BB", listOf("AA:BB", "CC:DD")))
    }
    ...
    fun `compat records are watch scoped and retain the legacy wire shape`() {
        ...
        val records = rockworkApplicationRecords(
    ...
    fun `compat UUIDs use Qt braces and resolution accepts UUID or store id`() {
        ...
        assertEquals("{44444444-4444-4444-8444-444444444444}", formatRockworkAppUuid(application.properties.id))
        assertSame(
            application,
            resolveRockworkInstalledApplication(installed, "{44444444-4444-4444-8444-444444444444}"),
        )
        assertSame(application, resolveRockworkInstalledApplication(installed, "store-id"))
        assertNull(resolveRockworkInstalledApplication(installed, "another-store-id"))
    }
    ...
    fun `compat order requires the exact installed set with Settings first`() {
        ...
        val valid = installed.map { formatRockworkAppUuid(it.properties.id) }

        assertEquals(installed.map { it.properties.id }, validateRockworkAppOrder(valid, installed))
        assertNull(validateRockworkAppOrder(valid.dropLast(1), installed))
        assertNull(validateRockworkAppOrder(listOf(valid[0], valid[1], valid[1]), installed))
        assertNull(validateRockworkAppOrder(listOf(valid[1], valid[0], valid[2]), installed))
        assertNull(
            validateRockworkAppOrder(
