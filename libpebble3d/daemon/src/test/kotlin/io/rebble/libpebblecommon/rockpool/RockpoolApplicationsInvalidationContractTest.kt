 ...
 * has no injectable signal sink for [RockpoolWatchObject], so keep this narrow
 * source-level contract until that seam exists.
 */
class RockpoolApplicationsInvalidationContractTest {
    @Test
    fun `watch refresh republishes Applications1 for a stable watch object`() {
        val source = Path.of(
            "src/main/kotlin/io/rebble/libpebblecommon/rockpool/RockpoolService.kt",
        ).readText()
        val methodStart = source.indexOf("fun propertiesChangedAll()")
        val methodEnd = source.indexOf("\n        override fun Connect()", methodStart)

        assertTrue(methodStart >= 0, "RockpoolWatchObject.propertiesChangedAll is missing")
