...
class RockpoolWeatherTest {
    ...
    fun `locations preserve the legacy av of string arrays and stable UUIDs`() {
        ...
        val settings = encodeRockpoolWeatherSettings(locations)
        assertEquals(locations, decodeRockpoolWeatherSettings(settings).getOrThrow())
    }

    @Test
    fun `locations reject malformed shapes coordinates duplicates and overflow`() {
        assertFailsWith<IllegalArgumentException> {
            parseRockpoolWeatherLocations(listOf(Variant(listOf("London", "1"), "as")))
        }
        assertFailsWith<IllegalArgumentException> {
            parseRockpoolWeatherLocations(listOf(Variant(listOf("London", "1", "2"), "av")))
        }
        assertFailsWith<IllegalArgumentException> {
            parseRockpoolWeatherLocations(listOf(location("London", "91", "0")))
        }
        assertFailsWith<IllegalArgumentException> {
        ...
        assertFailsWith<IllegalArgumentException> {
            parseRockpoolWeatherLocations((0..6).map { location("Location $it", "0", "0") })
        }
    }

    @Test
    fun `observation maps the legacy fields and validates bounds`() {
        val observation = parseRockpoolWeatherObservation(
            conditions = mapOf(
        ...
        assertFailsWith<IllegalArgumentException> {
            parseRockpoolWeatherObservation(
    ...
    fun `coordinator replays persisted weather on startup`() {
        val populated = RockpoolWeatherLocation(
            key = parseRockpoolWeatherLocations(listOf(location("London", "51.5", "-0.1")))
                .single().key,
            name = "London",
            latitude = "51.5",
            longitude = "-0.1",
        ...
        RockpoolWeatherCoordinator(
            loadSettings = { encodeRockpoolWeatherSettings(listOf(populated)) },
    ...
    fun `late persisted migration reloads and publishes the complete snapshot`() {
        ...
        settings = encodeRockpoolWeatherSettings(locations)
