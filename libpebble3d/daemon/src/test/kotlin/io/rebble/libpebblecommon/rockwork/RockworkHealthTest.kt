...
class RockworkHealthTest {
    ...
    fun `legacy health normalization rejects unknown malformed and unrepresentable values`() {
        listOf(
            ...
            assertFailsWith<IllegalArgumentException> {
                normalizeRockworkHealthParams(values)
            }
        }
    }
    ...
    fun `legacy health record preserves units and gender strings`() {
        val record = rockworkHealthParams(
            HealthSettings(
                heightMm = 1830,
                weightDag = 7200,
    ...
    fun `health coordinator serializes concurrent read merge updates`() {
        ...
        val coordinator = RockworkHealthCoordinator(libPebble) {
            delay(25)
            state.value = it
        }
        val start = CountDownLatch(1)
        val failure = AtomicReference<Throwable?>()
        val age = thread(name = "rockwork-health-test-age") {
            runCatching {
                start.await()
                coordinator.update({ it.copy(ageYears = 52) }, { true })
            }.onFailure { failure.compareAndSet(null, it) }
        }
        val units = thread(name = "rockwork-health-test-units") {
            runCatching {
                start.await()
    ...
    fun `health coordinator returns the serialized enable transition`() {
        ...
        val result = RockworkHealthCoordinator(libPebble) { state.value = it }.updateWithResult(
    ...
    fun `health sync throttle accepts only one request per minute`() {
        var nowNanos = 0L
        val throttle = RockworkHealthSyncThrottle { nowNanos }
