...
internal data class RockpoolWeatherLocation(
    ...
    val observation: RockpoolWeatherObservation? = null,
)

internal data class RockpoolWeatherObservation(
    val text: String,
    ...
    val source: RockpoolWeatherObservationSource = RockpoolWeatherObservationSource.EXTERNAL,
)

internal enum class RockpoolWeatherObservationSource {
    EXTERNAL,
    AUTOMATIC,
}

internal const val ROCKPOOL_WEATHER_SETTINGS_PREFIX = "weather.locations."
internal const val ROCKPOOL_MAX_WEATHER_LOCATIONS = 6

internal data class RockpoolWeatherFetchTarget(
    val key: Uuid,
    val name: String,
    val latitude: String,
    val longitude: String,
 ...
 */
internal class RockpoolWeatherCoordinator(
    ...
    fun setLocations(values: List<Variant<*>>): Boolean {
        val parsed = parseRockpoolWeatherLocations(values)
    ...
    @Synchronized
    fun automaticFetchTargets(): List<RockpoolWeatherFetchTarget> = state.mapIndexedNotNull { index, location ->
        if (location.observation?.source == RockpoolWeatherObservationSource.EXTERNAL) {
            return@mapIndexedNotNull null
        }
        val currentLocation = index == 0 && location.latitude == CURRENT_COORDINATE &&
            location.longitude == CURRENT_COORDINATE
        if (currentLocation) {
            return@mapIndexedNotNull RockpoolWeatherFetchTarget(
                key = location.key,
                name = location.name,
    ...
    fun inject(locationName: String, conditions: Map<String, Variant<*>>): Boolean {
        ...
        val observation = parseRockpoolWeatherObservation(conditions, nowEpochSeconds)
        val replacement = state.toMutableList().also {
...
internal fun parseRockpoolWeatherLocations(
    values: List<Variant<*>>,
): List<RockpoolWeatherLocation> {
    require(values.size <= ROCKPOOL_MAX_WEATHER_LOCATIONS) { "Too many weather locations" }
    ...
    return values.mapIndexed { index, variant ->
        ...
        RockpoolWeatherLocation(
            key = if (index == 0) CURRENT_LOCATION_UUID else rockpoolWeatherUuid(name),
            name = name,
            latitude = latitude,
            longitude = longitude,
        )
...
internal fun parseRockpoolWeatherObservation(
    ...
    val text = conditions["text"]?.let(::rockpoolWeatherString)
        ?: throw IllegalArgumentException("Weather text is required")
    requireWeatherString(text, MAX_FORECAST_BYTES, "Weather text")
    val temperature = conditions["temperature"]?.let(::rockpoolWeatherShort)
        ?: throw IllegalArgumentException("Weather temperature is required")
    val timestamp = conditions["ts"]?.let(::rockpoolWeatherTimestamp) ?: nowEpochSeconds()
    require(timestamp in 0..UInt.MAX_VALUE.toLong()) { "Weather timestamp is out of range" }
    return RockpoolWeatherObservation(
        text = text,
        temperature = temperature,
        timestampEpochSeconds = timestamp,
        todayHigh = conditions["today_hi"]?.let(::rockpoolWeatherShort) ?: Short.MAX_VALUE,
        todayLow = conditions["today_low"]?.let(::rockpoolWeatherShort) ?: Short.MAX_VALUE,
        todayIcon = conditions["today_icon"]?.let(::rockpoolWeatherType)
            ?: WeatherType.PartlyCloudy,
        tomorrowHigh = conditions["tomorrow_hi"]?.let(::rockpoolWeatherShort) ?: Short.MAX_VALUE,
        tomorrowLow = conditions["tomorrow_low"]?.let(::rockpoolWeatherShort) ?: Short.MAX_VALUE,
        tomorrowIcon = conditions["tomorrow_icon"]?.let(::rockpoolWeatherType)
            ?: WeatherType.PartlyCloudy,
    )
}

internal fun encodeRockpoolWeatherSettings(
    locations: List<RockpoolWeatherLocation>,
): Map<String, String> = buildMap {
    put("${ROCKPOOL_WEATHER_SETTINGS_PREFIX}count", locations.size.toString())
    locations.forEachIndexed { index, location ->
        val prefix = "$ROCKPOOL_WEATHER_SETTINGS_PREFIX$index."
...
internal fun decodeRockpoolWeatherSettings(
    values: Map<String, String>,
): Result<List<RockpoolWeatherLocation>> = runCatching {
    val count = values["${ROCKPOOL_WEATHER_SETTINGS_PREFIX}count"]?.toIntOrNull() ?: 0
    require(count in 0..ROCKPOOL_MAX_WEATHER_LOCATIONS)
    (0 until count).map { index ->
        val prefix = "$ROCKPOOL_WEATHER_SETTINGS_PREFIX$index."
        val location = RockpoolWeatherLocation(
        ...
        val observation = if (values["${prefix}observation"]?.toBooleanStrictOrNull() == true) {
            ...
            RockpoolWeatherObservation(
                ...
                source = when (values["${prefix}source"]) {
                    "automatic" -> RockpoolWeatherObservationSource.AUTOMATIC
                    else -> RockpoolWeatherObservationSource.EXTERNAL
                },
            )
        } else {
            null
        ...
        val validated = parseRockpoolWeatherLocations(locations.map {
...
private fun List<RockpoolWeatherLocation>.toWeatherData(): List<WeatherLocationData> =
...
private fun rockpoolWeatherString(variant: Variant<*>): String {
...
private fun rockpoolWeatherShort(variant: Variant<*>): Short {
    val value = rockpoolWeatherInteger(variant)
...
private fun rockpoolWeatherType(variant: Variant<*>): WeatherType {
    val code = rockpoolWeatherInteger(variant)
...
private fun rockpoolWeatherTimestamp(variant: Variant<*>): Long {
    if (variant.sig != "s") return rockpoolWeatherInteger(variant)
...
private fun rockpoolWeatherInteger(variant: Variant<*>): Long = when (variant.sig) {
...
private fun rockpoolWeatherUuid(name: String): Uuid {
