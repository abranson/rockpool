...
private val ROCKWORK_HEALTH_KEYS = setOf(
    "enabled",
    "age",
    "gender",
    "height",
...
internal typealias RockworkHealthUpdate = HealthSettingsUpdate
internal typealias RockworkHealthCoordinator = HealthSettingsCoordinator
 ...
 * burst of health requests.  The old Rockwork backend applied the same one-minute limit.
 */
internal class RockworkHealthSyncThrottle(
...
internal fun normalizeRockworkHealthParams(
    params: Map<String, Variant<*>>,
): Map<String, String> {
    require(params.isNotEmpty()) { "health settings cannot be empty" }
    require(params.keys.all { it in ROCKWORK_HEALTH_KEYS }) { "unknown health setting" }

    return buildMap {
        params.forEach { (key, variant) ->
            val value = when (key) {
                "enabled", "moreActive", "sleepMore" ->
                    variant.rockworkBoolean()?.toString()

                "age" -> variant.rockworkInt()?.takeIf { it in 1..130 }?.toString()
                "height" -> variant.rockworkInt()?.takeIf { it in 50..300 }?.toString()
                // libpebble3 stores decagrams in a signed short, hence 327 kg is the exact limit.
                "weight" -> variant.rockworkInt()?.takeIf { it in 10..327 }?.toString()
                "gender" -> variant.rockworkGender()?.toString()
                else -> null
            }
...
/** Reproduce the old org.rockwork record shape from libpebble3's actual current settings. */
internal fun rockworkHealthParams(settings: HealthSettings): Map<String, Variant<*>> = linkedMapOf(
...
private fun Variant<*>.rockworkBoolean(): Boolean? =
    if (sig == "b") value as? Boolean else null

private fun Variant<*>.rockworkGender(): Int? = when {
    ...
    else -> rockworkInt()?.takeIf {
        it == HealthGender.Female.ordinal || it == HealthGender.Male.ordinal
    }
}

private fun Variant<*>.rockworkInt(): Int? = when (sig) {
