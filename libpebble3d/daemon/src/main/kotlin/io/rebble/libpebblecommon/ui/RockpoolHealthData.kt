...
internal const val ROCKPOOL_HEALTH_MAX_DETAIL_SECONDS = 48L * 60L * 60L
 ...
 * historically returned by org.rockpool.Pebble. This is deliberately independent of D-Bus
 * objects so the data contract can be tested before the compatibility interface is expanded.
 */
internal class RockpoolHealthDataCoordinator(
    ...
    private fun boundedDayCount(startDate: LocalDate, endDate: LocalDate): Int? {
        ...
        var count = 0
        while (current < endDate && count <= ROCKPOOL_HEALTH_MAX_AVERAGE_DAYS) {
            current = current.plus(DatePeriod(days = 1))
            count++
        }
        require(current == endDate && count <= ROCKPOOL_HEALTH_MAX_AVERAGE_DAYS) {
            "Health date range exceeds $ROCKPOOL_HEALTH_MAX_AVERAGE_DAYS days"
        }
        return count
    }
    ...
    private fun validateEpochRange(start: Long, end: Long) {
        ...
        require(duration <= ROCKPOOL_HEALTH_MAX_DETAIL_SECONDS) {
            "Health range exceeds $ROCKPOOL_HEALTH_MAX_DETAIL_SECONDS seconds"
        }
    }
}
