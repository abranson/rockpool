...
import io.rebble.libpebblecommon.connection.CalendarSyncOutcome
...
/**
 * The generic, versioned D-Bus surface. It intentionally has no dependency on
 * the org.rockpool UI facade other than the shared settings backend.
 ...
 */
internal class LibPebble3Service(
    ...
    private val sendTextConfiguration: SendTextConfigurationCoordinator,
    private val classicAvailable: Boolean,
) {
    private val logger = Logger.withTag("LibPebble3Service")
    ...
    init {
        ...
        sendTextConfiguration.addListener(::messagingFavoritesChanged)
    ...
    private val operations = LinkedHashMap<String, LibPebble3OperationObject>()

    private data class ExportedWatch(
        val path: String,
        val objectId: String,
        val watch: LibPebble3WatchObject,
        val devConnectionObserver: DevConnectionStateObserver,
        val runningAppObserver: RunningAppStateObserver,
        val firmwareProgressObserver: FirmwareProgressObserver,
    )
            ...
            try {
                while (connection == null) {
                    try {
                        ...
                        logger.w { "io.rebble.libpebble3 unavailable (${e.message}); retrying in 5s" }
                        delay(5.seconds)
                    }
                }
            } finally {
    ...
    private suspend fun connectAndExport() = withContext(Dispatchers.IO) {
        ...
        // connection, exposing all objects through both io.rebble.libpebble3 and org.rockpool.
        ...
        logger.i { "io.rebble.libpebble3 exported on an isolated session-bus connection" }
    ...
    private fun finishConnectionLoss(conn: DBusConnection) {
        ...
        logger.w { "io.rebble.libpebble3 session-bus connection lost; reconnecting" }
        ensureConnection()
    }
        ...
        scope.launch {
            libPebble.watches.collect { devices ->
                ...
                val publication = managedObjects.mutate {
                    ...
                    known.forEach { (id, device) ->
                        ...
                        if (current == null) {
                            ...
                            val obj = LibPebble3WatchObject(id, path, device.identifier.asString)
                            val observer = DevConnectionStateObserver(scope) {
                                obj.propertiesChanged(DEVELOPER_INTERFACE, setOf("LocalEnabled"))
                            }
                            val runningAppObserver = RunningAppStateObserver(scope) {
                                obj.propertiesChanged(APPLICATIONS_INTERFACE, setOf("Applications"))
                            }
                            val firmwareProgressObserver = FirmwareProgressObserver(scope) {
                                obj.propertiesChanged(FIRMWARE_INTERFACE, setOf("UpdateProgress"))
                            }
                            val exported = ExportedWatch(
                                path,
                                id,
                                obj,
                                observer,
                                runningAppObserver,
                                firmwareProgressObserver,
                            )
                            publish(
                ...
                removed.forEach { exported ->
                    ...
                    exported.runningAppObserver.close()
                    exported.firmwareProgressObserver.close()
                    emitRemoved(
                ...
                synchronized(objectsLock) { watches.values.toList() }
                    .forEach { exported ->
                        ...
                        val connected = known[exported.objectId] as? ConnectedPebbleDevice
                        exported.runningAppObserver.update(
                            source = connected,
                            state = connected?.runningApp,
                        )
                        val firmwareProgress = (
                            common?.firmwareUpdateState
                                as? FirmwareUpdater.FirmwareUpdateStatus.InProgress
                            )?.progress
                        exported.firmwareProgressObserver.update(
                            source = firmwareProgress,
                            state = firmwareProgress,
                        )
    ...
    private fun messagingFavoritesChanged() {
        synchronized(objectsLock) { watches.values.map { it.watch } }
            .forEach { it.propertiesChanged(MESSAGING_INTERFACE, setOf("Favorites")) }
    }
    ...
    private abstract inner class LibPebble3Object(
    ...
    private inner class ManagerObject : LibPebble3Object(MANAGER_PATH), LibPebble3Manager1,
        LibPebble3Discovery1, LibPebble3Account1 {
    ...
    private inner class PlatformObject : LibPebble3Object(PLATFORM_PATH), LibPebble3Platform1 {
    ...
    private inner class LibPebble3WatchObject(
        private val objectId: String,
        path: String,
        address: String,
    ) : LibPebble3Object(path),
        LibPebble3Watch1,
        LibPebble3Firmware1,
        LibPebble3Applications1,
        LibPebble3Timeline1,
        LibPebble3Notifications1,
        LibPebble3Messaging1,
        LibPebble3Health1,
        LibPebble3Profiles1,
        LibPebble3Screenshots1,
        LibPebble3Logs1,
        LibPebble3Developer1 {
        @Volatile
        ...
        private fun primarySendTextFavorites(): List<Map<String, Variant<*>>> =
            sendTextFavoriteRecords(sendTextConfiguration.favorites()).map { values ->
                record(*values.map { (name, value) -> name to Variant(value) }.toTypedArray())
            }

        private suspend fun installedApplication(applicationId: Uuid): LockerWrapper? {
            val application = libPebble.getLockerApp(applicationId).first() ?: return null
            val watchType = (device() as? KnownPebbleDevice)?.watchType?.watchType ?: return null
            return application.takeIf { rockpoolApplicationIsInstalledOnWatch(it, watchType) }
        }

        private fun pkjsSession(device: ConnectedPebbleDevice, applicationId: Uuid): PKJSApp? =
            device.currentCompanionAppSessions.value
                .filterIsInstance<PKJSApp>()
                .singleOrNull { it.uuid == applicationId }

        private fun firmwareStatus(): PrimaryFirmwareStatus {
            val current = device()
            val connected = current as? CommonConnectedDevice
            return primaryFirmwareStatus(
                connected = connected != null,
                recovery = current is ConnectedPebbleDeviceInRecovery,
                check = connected?.firmwareUpdateAvailable,
                update = connected?.firmwareUpdateState,
            )
        }

        private fun firmwareCheckStates() = libPebble.watches
            .map { devices ->
                (devices.firstOrNull { it.matchesAddress(address) } as? CommonConnectedDevice)
                    ?.firmwareUpdateAvailable
            }
            .filterNotNull()

        private fun firmwareInstallStates() = libPebble.watches
            .map { devices ->
                (devices.firstOrNull { it.matchesAddress(address) } as? CommonConnectedDevice)
                    ?.firmwareUpdateState
            }

        private fun languageInstallStates() = libPebble.watches
            .map { devices ->
                (devices.firstOrNull { it.matchesAddress(address) } as? ConnectedPebbleDevice)
        ...
        override fun managedInterfaces(): Map<String, Map<String, Variant<*>>> {
            ...
            return mapOf(
                ...
                FIRMWARE_INTERFACE to mapOf(
                    ...
                    "Recovery" to Variant(firmware.recovery),
                    "CheckingForUpdate" to Variant(firmware.checking),
                    "UpdateAvailable" to Variant(firmware.updateAvailable),
                    "CandidateVersion" to Variant(firmware.candidateVersion),
                    "ReleaseNotes" to Variant(firmware.releaseNotes),
                    "UpdateState" to Variant(firmware.updateState),
                    "UpdateProgress" to Variant(firmware.updateProgress),
                ),
                APPLICATIONS_INTERFACE to mapOf(
                    "Applications" to Variant(
                        known?.let {
                            rockpoolApplicationRecords(
                                lockerApplications,
                                it.watchType.watchType,
                                (connected as? ConnectedPebbleDevice)?.runningApp?.value,
                            )
                        }
                            .orEmpty(),
                        "aa{sv}",
                ...
                MESSAGING_INTERFACE to mapOf(
                    ...
                    "Favorites" to Variant(primarySendTextFavorites(), "aa{sv}"),
                ),
        ...
        override fun InstallFirmware(firmware: FileDescriptor): DBusPath = descriptorInstallOperation(
            descriptor = firmware,
            kind = "watch.install-firmware",
            suffix = ".pbz",
            maximumBytes = MAX_FIRMWARE_INSTALL_BYTES,
        ) { staged ->
            val connected = device() as? CommonConnectedDevice
            if (connected == null) {
                fail(ERROR_NOT_CONNECTED, "firmware installation needs a connected watch")
                return@descriptorInstallOperation
            }
            val initial = connected.firmwareUpdateState
            if (initial is FirmwareUpdater.FirmwareUpdateStatus.Active) {
                fail(ERROR_BUSY, "a firmware installation is already active")
                return@descriptorInstallOperation
            }
            val firmwarePath = kotlinx.io.files.Path(staged.toString())
            try {
                val running = connected.watchInfo.runningFwVersion
                val updateToSlot = when {
                    running.isDualSlot && running.isSlot0 -> 1
                    running.isDualSlot -> 0
                    else -> null
                }
                PbzFirmware(firmwarePath).findManifestFor(updateToSlot)
            } catch (e: Exception) {
                logger.w(e) { "received firmware package is invalid" }
                fail(ERROR_INVALID_ARGUMENT, "firmware package is invalid")
                return@descriptorInstallOperation
            }
            setProgress(0.1)
            if (!beginCommit()) return@descriptorInstallOperation
            val completion = withContext(NonCancellable) {
                withTimeoutOrNull(FIRMWARE_INSTALL_TIMEOUT) {
                    awaitPrimaryFirmwareInstall(
                        initial = initial,
                        states = firmwareInstallStates(),
                    ) {
                        connected.sideloadFirmware(firmwarePath)
                    }
                }
            }
            when (completion) {
                PrimaryInstallCompletion.SUCCEEDED -> succeed()
                PrimaryInstallCompletion.DISCONNECTED ->
                    fail(ERROR_TRANSPORT_FAILED, "watch disconnected during firmware installation")
                PrimaryInstallCompletion.FAILED ->
                    fail(ERROR_TRANSPORT_FAILED, "firmware installation failed")
                null -> fail(ERROR_TRANSPORT_FAILED, "firmware installation timed out")
            }
        }

        override fun InstallLanguagePack(languagePack: FileDescriptor): DBusPath =
            descriptorInstallOperation(
                descriptor = languagePack,
                kind = "watch.install-language",
                suffix = ".pbl",
                maximumBytes = MAX_LANGUAGE_INSTALL_BYTES,
            ) { staged ->
                val connected = device() as? ConnectedPebbleDevice
                if (connected == null) {
                    fail(ERROR_NOT_CONNECTED, "language installation needs a connected compatible watch")
                    return@descriptorInstallOperation
                }
                val initial = connected.languagePackInstallState
                if (initial !is LanguagePackInstallState.Idle) {
                    fail(ERROR_BUSY, "a language installation is already active")
                    return@descriptorInstallOperation
                }
                val languagePath = kotlinx.io.files.Path(staged.toString())
                setProgress(0.1)
                if (!beginCommit()) return@descriptorInstallOperation
                val completion = withContext(NonCancellable) {
                    withTimeoutOrNull(LANGUAGE_INSTALL_TIMEOUT) {
                        awaitPrimaryLanguageInstall(
                            initial = initial,
                            states = languageInstallStates(),
                            expectedName = PRIMARY_LANGUAGE_PACK_NAME,
                        ) {
                            connected.installLanguagePack(languagePath, PRIMARY_LANGUAGE_PACK_NAME)
                        }
                    }
                }
                when (completion) {
                    PrimaryInstallCompletion.SUCCEEDED -> succeed()
                    PrimaryInstallCompletion.DISCONNECTED ->
                        fail(ERROR_TRANSPORT_FAILED, "watch disconnected during language installation")
                    PrimaryInstallCompletion.FAILED ->
                        fail(ERROR_TRANSPORT_FAILED, "language installation failed")
                    null -> fail(ERROR_TRANSPORT_FAILED, "language installation timed out")
                }
            }

        override fun CheckForUpdate(force: Boolean): DBusPath =
            operation("watch.check-firmware-update") {
                val connected = device() as? CommonConnectedDevice
                if (connected == null) {
                    fail(ERROR_NOT_CONNECTED, "watch is not connected")
                    return@operation
                }
                val initial = connected.firmwareUpdateAvailable
                if (!beginCommit()) return@operation
                setProgress(0.1)
                val completed: FirmwareUpdateCheckState? = withContext(NonCancellable) {
                    try {
                        withTimeoutOrNull(FIRMWARE_CHECK_TIMEOUT) {
                            awaitFirmwareCheck(initial, firmwareCheckStates()) {
                                connected.checkforFirmwareUpdate(force)
                            }
                        }
                    } catch (e: Exception) {
                        logger.w(e) { "firmware update check failed" }
                        null
                    }
                }
                if (completed == null) {
                    fail(ERROR_PROVIDER_UNAVAILABLE, "firmware update check did not complete")
                    return@operation
                }
                when (completed.result) {
                    is FirmwareUpdateCheckResult.UpdateCheckFailed -> {
                        fail(ERROR_PROVIDER_UNAVAILABLE, "firmware update check failed")
                        return@operation
                    }
                    null -> {
                        fail(ERROR_PROVIDER_PROTOCOL, "firmware update check returned no result")
                        return@operation
                    }
                    else -> Unit
                }
                ...
                succeed(
                    mapOf(
                        "updateAvailable" to Variant(status.updateAvailable),
                        "candidateVersion" to Variant(status.candidateVersion),
                        "releaseNotes" to Variant(status.releaseNotes),
                        "recovery" to Variant(status.recovery),
                    ),
                )
            }

        override fun Install(pbw: FileDescriptor): DBusPath = descriptorInstallOperation(
            descriptor = pbw,
            kind = "watch.install-app",
            suffix = ".pbw",
            maximumBytes = MAX_PBW_INSTALL_BYTES,
        ) { staged ->
            val connected = device() as? ConnectedPebbleDevice
            if (connected == null) {
                fail(ERROR_NOT_CONNECTED, "application installation needs a connected compatible watch")
                return@descriptorInstallOperation
            }
            val knownAddresses = libPebble.watches.value
                .filterIsInstance<KnownPebbleDevice>()
                .map { it.identifier.asString.uppercase() }
                .distinct()
            if (knownAddresses != listOf(address.uppercase())) {
                fail(
                    ERROR_NOT_SUPPORTED,
                    "application installation cannot address one watch while the locker is shared",
                )
                return@descriptorInstallOperation
            }
            val pbwPath = kotlinx.io.files.Path(staged.toString())
            val applicationId = try {
                val candidate = PbwApp(pbwPath)
                require(candidate.bestVariantFor(connected.watchType.watchType) != null)
                Uuid.parse(candidate.info.uuid)
            } catch (e: Exception) {
                logger.w(e) { "received PBW is invalid or incompatible" }
                fail(ERROR_INVALID_ARGUMENT, "PBW is invalid or incompatible with this watch")
                return@descriptorInstallOperation
            }
            setProgress(0.25)
            if (!beginCommit()) return@descriptorInstallOperation
            val installed = withContext(NonCancellable) {
                withTimeoutOrNull(APP_INSTALL_TIMEOUT) {
                    libPebble.sideloadApp(pbwPath)
                }
            } ?: false
            if (!installed) {
                fail(ERROR_TRANSPORT_FAILED, "application installation failed")
                return@descriptorInstallOperation
            }
        ...
        override fun Launch(uuid: String): DBusPath = operation("watch.launch-app") {
            val applicationId = parseCanonicalApplicationUuid(uuid)
            if (applicationId == null) {
                fail(ERROR_INVALID_ARGUMENT, "application UUID is invalid")
                return@operation
            }
            if (installedApplication(applicationId) == null) {
                fail(ERROR_NOT_FOUND, "application is not installed on this watch")
                return@operation
            }
            val connected = device() as? ConnectedPebbleDevice
            ...
            if (connected.runningApp.value == applicationId) {
                succeed(
                    mapOf(
                        "uuid" to Variant(uuid),
                        "alreadyRunning" to Variant(true),
                    ),
                )
                return@operation
            }
            if (!beginCommit()) return@operation
            val launched = withContext(NonCancellable) {
                withTimeoutOrNull(APP_RUNTIME_TIMEOUT) {
                    try {
                        connected.launchApp(applicationId)
                        true
                    } catch (e: Exception) {
                        logger.w(e) { "could not launch application $applicationId" }
                        false
                    }
                } ?: false
            }
            if (!launched) {
                fail(ERROR_TRANSPORT_FAILED, "watch did not confirm application launch")
                return@operation
            }
            succeed(
                mapOf(
                    "uuid" to Variant(uuid),
                    "alreadyRunning" to Variant(false),
                ),
            )
        }

        override fun Close(uuid: String): DBusPath = operation("watch.close-app") {
            val applicationId = parseCanonicalApplicationUuid(uuid)
            if (applicationId == null) {
                fail(ERROR_INVALID_ARGUMENT, "application UUID is invalid")
                return@operation
            ...
            if (connected.runningApp.value != applicationId) {
                fail(ERROR_NOT_FOUND, "application is not running on this watch")
                return@operation
            }
            if (!beginCommit()) return@operation
            val closed = withContext(NonCancellable) {
                withTimeoutOrNull(APP_RUNTIME_TIMEOUT) {
                    try {
                        connected.stopApp(applicationId)
                        connected.runningApp.first { it != applicationId }
                        true
                    } catch (e: Exception) {
                        logger.w(e) { "could not close application $applicationId" }
                        false
                    }
                } ?: false
            }
            if (!closed) {
                fail(ERROR_TRANSPORT_FAILED, "watch did not confirm application close")
                return@operation
            }
            succeed(mapOf("uuid" to Variant(uuid)))
        }

        override fun RequestConfiguration(uuid: String): DBusPath =
            operation("watch.request-app-configuration") {
                val applicationId = parseCanonicalApplicationUuid(uuid)
                if (applicationId == null) {
                    fail(ERROR_INVALID_ARGUMENT, "application UUID is invalid")
                    return@operation
                }
                val application = installedApplication(applicationId)
                if (application == null) {
                    fail(ERROR_NOT_FOUND, "application is not installed on this watch")
                    return@operation
                }
                if (application !is LockerWrapper.NormalApp || !application.configurable) {
                    fail(ERROR_NOT_SUPPORTED, "application has no configuration page")
                    return@operation
                }
                val connected = device() as? ConnectedPebbleDevice
                if (connected == null) {
                ...
                val url = withContext(NonCancellable) {
                    withTimeoutOrNull(APP_CONFIGURATION_TIMEOUT) {
                        try {
                            val session = pkjsSession(connected, applicationId) ?: run {
                                connected.launchApp(applicationId)
                                connected.currentCompanionAppSessions
                                    .map { sessions ->
                                        sessions.filterIsInstance<PKJSApp>()
                                            .singleOrNull { it.uuid == applicationId }
                                    }
                                    .filterNotNull()
                                    .first()
                            ...
                            logger.w(e) {
                                "could not request configuration for $applicationId"
                            }
                            null
                        }
                    }
                ...
                if (url == null) {
                    fail(ERROR_TRANSPORT_FAILED, "application returned no configuration URL")
                    return@operation
                }
                if (!validApplicationConfigurationUrl(url)) {
                    fail(ERROR_PROVIDER_PROTOCOL, "application returned an invalid configuration URL")
                    return@operation
                }
                succeed(
                    mapOf(
        ...
        override fun SubmitConfiguration(uuid: String, result: String): DBusPath =
            operation("watch.submit-app-configuration") {
                val applicationId = parseCanonicalApplicationUuid(uuid)
                if (applicationId == null || !validApplicationConfigurationResult(result)) {
                    fail(ERROR_INVALID_ARGUMENT, "invalid application UUID or configuration result")
                    return@operation
                }
                val application = installedApplication(applicationId)
                if (application == null) {
                ...
                val session = pkjsSession(connected, applicationId)
                if (session == null) {
                    fail(ERROR_NOT_FOUND, "application configuration session is no longer active")
                    return@operation
                }
                if (!beginCommit()) return@operation
                withContext(NonCancellable) { session.triggerOnWebviewClosed(result) }
                succeed(mapOf("uuid" to Variant(uuid)))
            }

        override fun Sync(): DBusPath = operation("watch.timeline-sync") {
            setProgress(0.1)
            val outcome = try {
                libPebble.syncCalendars(::beginCommit)
            } catch (e: CancellationException) {
                throw e
            } catch (e: Exception) {
                logger.w(e) { "explicit calendar sync failed" }
                fail(ERROR_PROVIDER_UNAVAILABLE, "calendar source sync failed")
                return@operation
            }
            when (outcome) {
                CalendarSyncOutcome.SourceUnavailable ->
                    fail(ERROR_PROVIDER_UNAVAILABLE, "calendar source is unavailable")
                is CalendarSyncOutcome.Completed -> succeed(
                    mapOf(
                        "calendarCount" to Variant(outcome.calendarCount),
                        "eventCount" to Variant(outcome.eventCount),
                        "reminderCount" to Variant(outcome.reminderCount),
                        "calendarEnabled" to Variant(outcome.calendarEnabled),
                    ),
                )
            }
        }
        ...
        override fun SetFavorites(favorites: List<Map<String, Variant<*>>>): DBusPath =
            operation("watch.set-message-favorites") {
                val values = normalizeSendTextFavorites(favorites) ?: return@operation
                when (sendTextConfiguration.replaceFavorites(values, ::beginCommit)) {
                    SendTextFavoritesUpdateResult.Completed -> Unit
                    SendTextFavoritesUpdateResult.Cancelled -> return@operation
                    SendTextFavoritesUpdateResult.InvalidArgument -> {
                        fail(ERROR_INVALID_ARGUMENT, "invalid Send Text favorites")
                        return@operation
                    }
                    SendTextFavoritesUpdateResult.PersistenceFailed -> {
                        fail(ERROR_IO, "could not save Send Text favorites")
                        return@operation
                    }
                    SendTextFavoritesUpdateResult.ActivationFailed -> {
                        fail(ERROR_IO, "favorites were saved but watch activation is pending")
                        return@operation
                    }
                }
                succeed(mapOf("methods" to Variant(favorites.size)))
            }

        override fun SendText(methodId: String, text: String): DBusPath =
            operation("watch.send-text") {
                val parsedMethodId = runCatching { Uuid.parse(methodId) }.getOrNull()
                if (parsedMethodId == null || parsedMethodId.toString() != methodId ||
                    !validOutgoingText(text)
                ) {
                    fail(ERROR_INVALID_ARGUMENT, "invalid method ID or message text")
                    return@operation
                }
                if (!beginCommit()) return@operation
                when (withContext(NonCancellable) {
                    sendTextConfiguration.sendText(parsedMethodId, text)
                }) {
                    SendTextDispatchResult.Sent -> succeed(
                        mapOf("methodId" to Variant(methodId)),
                    )
                    SendTextDispatchResult.InvalidArgument ->
                        fail(ERROR_INVALID_ARGUMENT, "invalid message text")
                    SendTextDispatchResult.UnknownMethod ->
                        fail(ERROR_NOT_FOUND, "Send Text method is not configured")
                    SendTextDispatchResult.Unavailable ->
                        fail(ERROR_PROVIDER_UNAVAILABLE, "system messaging is unavailable")
                    SendTextDispatchResult.Failed ->
                        fail(ERROR_IO, "system messaging rejected the message")
                }
            }
        ...
        private fun LibPebble3OperationObject.normalizeNotificationFilters(
        ...
        private fun LibPebble3OperationObject.normalizeCannedResponses(
        ...
        private fun LibPebble3OperationObject.normalizeSendTextFavorites(
            favorites: List<Map<String, Variant<*>>>,
        ): Map<String, List<String>>? {
            if (favorites.size > SEND_TEXT_MAX_FAVORITES) {
                fail(ERROR_INVALID_ARGUMENT, "too many Send Text methods")
                return null
            }
            val values = linkedMapOf<String, MutableList<String>>()
            val routes = HashSet<String>()
            favorites.forEach { favorite ->
                if (favorite.keys.any { it !in SEND_TEXT_FAVORITE_FIELDS }) {
                    fail(ERROR_INVALID_ARGUMENT, "unknown Send Text favorite field")
                    return null
                }
                val name = favorite.stringValue("name")
                val account = favorite.stringValue("account")
                val recipient = favorite.stringValue("recipient")
                if (name.isNullOrBlank() || account.isNullOrBlank() || recipient.isNullOrBlank() ||
                    SETTINGS_LIST_SEPARATOR in name || SETTINGS_LIST_SEPARATOR in account ||
                    SETTINGS_LIST_SEPARATOR in recipient
                ) {
                    fail(ERROR_INVALID_ARGUMENT, "invalid Send Text favorite")
                    return null
                }
                val encodedRoute = "$account:$recipient"
                val contactId = rockpoolSendTextContactUuid(name).toString()
                val methodId = rockpoolSendTextMethodUuid(encodedRoute).toString()
                val displayRecipient = favorite.optionalStringValue("displayRecipient")
                if (!favorite.matchesOptionalString("contactId", contactId) ||
                    !favorite.matchesOptionalString("methodId", methodId) ||
                    ("displayRecipient" in favorite && displayRecipient != recipient) ||
                    !routes.add(encodedRoute)
                ) {
                    fail(ERROR_INVALID_ARGUMENT, "inconsistent Send Text favorite identity")
                    return null
                }
                values.getOrPut(name) { mutableListOf() } += encodedRoute
            }
            return values.mapValues { it.value.toList() }
        }

        private fun LibPebble3OperationObject.normalizeHealthSettings(
        ...
        private fun LibPebble3OperationObject.normalizeProfiles(
        ...
        private fun Map<String, Variant<*>>.matchesOptionalString(
            name: String,
            expected: String,
        ): Boolean = when (val value = this[name]?.value) {
            null -> true
            is String -> value == expected
            else -> false
        }

        private fun descriptorInstallOperation(
            descriptor: FileDescriptor,
            kind: String,
            suffix: String,
            maximumBytes: Long,
            install: suspend LibPebble3OperationObject.(java.nio.file.Path) -> Unit,
        ): DBusPath {
            val input = try {
                duplicateInstallFile(descriptor)
            } catch (e: Exception) {
                logger.w(e) { "$kind received an unusable file descriptor" }
                return operation(kind) {
                    fail(ERROR_INVALID_ARGUMENT, "installation file descriptor is invalid")
                }
            }
            return try {
                operation(
                    kind = kind,
                    onCompletion = { runCatching(input::close) },
                ) {
                    val staged = try {
                        stageInstallFile(input, suffix, maximumBytes)
                    } catch (e: InvalidInstallFileException) {
                        fail(ERROR_INVALID_ARGUMENT, e.message ?: "installation file is invalid")
                        return@operation
                    } catch (e: IOException) {
                        logger.w(e) { "$kind could not stage its input" }
                        fail(ERROR_IO, "installation file could not be read")
                        return@operation
                    }
                    try {
                        install(staged)
                        ...
                        runCatching { Files.deleteIfExists(staged) }
                            .onFailure { logger.w(it) { "$kind could not remove its staged input" } }
                    }
                }
            } catch (e: Exception) {
    ...
    private inner class LibPebble3OperationObject(
        val path: String,
        private val kind: String,
    ) : LibPebble3Object(path), LibPebble3Operation1 {
        ...
        fun prepare(
            onCompletion: (() -> Unit)?,
            task: suspend LibPebble3OperationObject.() -> Unit,
        ) {
        ...
        fun succeed(values: Map<String, Variant<*>> = emptyMap()) {
            ...
            if (shouldSignal) emitSignal(LibPebble3Operation1.Completed(path, true))
        }
        ...
        fun fail(stableError: String, detail: String) {
            ...
            if (shouldSignal) emitSignal(LibPebble3Operation1.Completed(path, false))
        }
    ...
    private fun operation(
        ...
        onPrepared: ((LibPebble3OperationObject) -> Unit)? = null,
        task: suspend LibPebble3OperationObject.() -> Unit,
    ): DBusPath {
        val path = "$OPERATIONS_PATH/op_${UUID.randomUUID().toString().replace("-", "")}"
        val operation = LibPebble3OperationObject(path, kind)
    ...
    companion object {
        private const val BUS_NAME = "io.rebble.libpebble3"
        private const val ROOT_PATH = "/io/rebble/libpebble3"
        ...
        private const val SEND_TEXT_MAX_FAVORITES = 10
        private const val MAX_SETTING_STRING = 512
        private const val MAX_PBW_INSTALL_BYTES = 32L * 1024L * 1024L
        private const val MAX_FIRMWARE_INSTALL_BYTES = 64L * 1024L * 1024L
        private const val MAX_LANGUAGE_INSTALL_BYTES = 16L * 1024L * 1024L
        private const val PRIMARY_LANGUAGE_PACK_NAME = "primary-language-pack"
        private const val UINT32_MAX = 4_294_967_295L

        private const val OBJECT_MANAGER_INTERFACE = "org.freedesktop.DBus.ObjectManager"
        private const val MANAGER_INTERFACE = "io.rebble.libpebble3.Manager1"
        private const val DISCOVERY_INTERFACE = "io.rebble.libpebble3.Discovery1"
        private const val ACCOUNT_INTERFACE = "io.rebble.libpebble3.Account1"
        private const val PLATFORM_INTERFACE = "io.rebble.libpebble3.Platform1"
        private const val WATCH_INTERFACE = "io.rebble.libpebble3.Watch1"
        private const val FIRMWARE_INTERFACE = "io.rebble.libpebble3.Firmware1"
        private const val APPLICATIONS_INTERFACE = "io.rebble.libpebble3.Applications1"
        private const val TIMELINE_INTERFACE = "io.rebble.libpebble3.Timeline1"
        private const val NOTIFICATIONS_INTERFACE = "io.rebble.libpebble3.Notifications1"
        private const val MESSAGING_INTERFACE = "io.rebble.libpebble3.Messaging1"
        private const val HEALTH_INTERFACE = "io.rebble.libpebble3.Health1"
        private const val PROFILES_INTERFACE = "io.rebble.libpebble3.Profiles1"
        private const val SCREENSHOTS_INTERFACE = "io.rebble.libpebble3.Screenshots1"
        private const val LOGS_INTERFACE = "io.rebble.libpebble3.Logs1"
        private const val DEVELOPER_INTERFACE = "io.rebble.libpebble3.Developer1"
        private const val OPERATION_INTERFACE = "io.rebble.libpebble3.Operation1"

        private const val ERROR_INVALID_ARGUMENT = "io.rebble.libpebble3.Error.InvalidArgument"
        private const val ERROR_NOT_SUPPORTED = "io.rebble.libpebble3.Error.NotSupported"
        private const val ERROR_NOT_FOUND = "io.rebble.libpebble3.Error.NotFound"
        private const val ERROR_NOT_CONNECTED = "io.rebble.libpebble3.Error.NotConnected"
        private const val ERROR_PAIRING_FAILED = "io.rebble.libpebble3.Error.PairingFailed"
        private const val ERROR_PROVIDER_UNAVAILABLE = "io.rebble.libpebble3.Error.ProviderUnavailable"
        private const val ERROR_PROVIDER_PROTOCOL = "io.rebble.libpebble3.Error.ProviderProtocol"
        private const val ERROR_AUTHENTICATION_FAILED = "io.rebble.libpebble3.Error.AuthenticationFailed"
        private const val ERROR_BUSY = "io.rebble.libpebble3.Error.Busy"
        private const val ERROR_INTERNAL = "io.rebble.libpebble3.Error.Internal"

        private val CONNECTION_TIMEOUT = 45.seconds
        private val APP_RUNTIME_TIMEOUT = 15.seconds
        private val APP_INSTALL_TIMEOUT = 2.minutes
        private val APP_CONFIGURATION_TIMEOUT = 30.seconds
        private val FIRMWARE_CHECK_TIMEOUT = 45.seconds
        private val FIRMWARE_INSTALL_TIMEOUT = 30.minutes
        private val LANGUAGE_INSTALL_TIMEOUT = 10.minutes
        ...
        private val SEND_TEXT_FAVORITE_FIELDS = setOf(
            "contactId",
            "methodId",
            "name",
            "account",
