...
internal class RockworkPebbleObject(
    ...
    private val healthSyncThrottle = RockworkHealthSyncThrottle()
    ...
    private fun canMutateGlobalAppState(operation: String): Boolean {
        ...
        val allowed = rockworkGlobalAppMutationAllowed(address, knownAddresses)
        if (!allowed) {
            logger.w {
    ...
    override fun LanguageVersion(): String = rockworkLanguageStatus(device()).version
    override fun Model(): Int = rockworkWatchModel(
    ...
    override fun LoadLanguagePack(pblFile: String) {
        ...
        val source = try {
            parseRockworkLanguagePackSource(pblFile)
    ...
    override fun SetNotificationFilter(sourceId: String, enabled: Int) {
        ...
        // RockworkService observes both the application Flow and coordinator changes. It emits
    ...
    override fun getLogLevel(): Int = normalizedRockworkLogLevel(settings.getInt("logLevel", 1))

    private fun setJournalLogLevel(level: Int) {
        val normalized = normalizedRockworkLogLevel(level)
        settings.set("logLevel", normalized)
        Logger.setMinSeverity(rockworkLogSeverity(normalized))
    }

    override fun DumpLogs(fileName: String) {
        // Keep the argument for Rockwork D-Bus compatibility, but never trust it as an output path.
        scope.launch {
            if (!watchLogDumpMutex.tryLock()) {
                emit(RockworkPebble.LogsDumped(path, false))
                return@launch
            }
            val ok = try {
    ...
    private suspend fun installedLocker(): List<LockerWrapper> {
        ...
        return rockworkInstalledApplications(locker(), watchType)
    }
    ...
    override fun InstalledAppIds(): List<String> = runBlocking {
        installedLocker().map { formatRockworkAppUuid(it.properties.id) }
    }
    ...
    override fun InstalledApps(): List<Variant<*>> = runBlocking {
        ...
        rockworkApplicationRecords(locker(), watchType)
    }

    override fun RemoveApp(id: String) {
        scope.launch {
            if (!canMutateGlobalAppState("RemoveApp")) return@launch
            val application = resolveRockworkInstalledApplication(installedLocker(), id)
            if (application == null) {
    ...
    override fun ConfigurationURL(uuid: String) {
        scope.launch {
            val application = resolveRockworkInstalledApplication(installedLocker(), uuid)
            if (application == null) {
    ...
    override fun SetAppOrder(newList: List<String>) {
        scope.launch {
            ...
            val order = validateRockworkAppOrder(newList, installed)
            if (order == null) {
    ...
    override fun SendAppData(uuid: String, data: Map<String, Variant<*>>) {
        ...
        val dictionary = try {
            rockworkAppMessageDictionary(data, sessionKeys, maximumPayloadSize)
        } catch (e: IllegalArgumentException) {
            throw invalidArgument(e.message ?: "Invalid AppMessage")
        }
    ...
    override fun RequestScreenshot() {
        scope.launch {
            try {
                ...
                emit(RockworkPebble.ScreenshotAdded(path, record.path))
            } catch (e: CancellationException) {
                throw e
            } catch (e: Exception) {
    ...
    override fun RemoveScreenshot(filename: String) {
        ...
        if (removed != null) {
            emit(RockworkPebble.ScreenshotRemoved(path, removed.path))
        }
    }
    ...
    override fun HealthParams(): Map<String, Variant<*>> = try {
        rockworkHealthParams(healthCoordinator.current())
    } catch (e: Exception) {
    ...
    override fun SetHealthParams(params: Map<String, Variant<*>>) {
        val normalized = try {
            normalizeRockworkHealthParams(params)
        } catch (e: IllegalArgumentException) {
