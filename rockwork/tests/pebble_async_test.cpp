/*
 * Regression coverage for asynchronous org.rockwork.Pebble bootstrap calls.
 *
 * Run with run-pebbles-async-test.sh so org.rockwork is private to this test.
 */
#include "pebble.h"
...
namespace {
const char serviceName[] = "org.rockwork";
const char accountServiceName[] = "org.rockpool";
const char watchPath[] = "/org/rockwork/Watch/test";
const char watchInterface[] = "org.rockwork.Pebble";
const char accountPath[] = "/org/rockpool/Manager";
const char accountInterface[] = "org.rockpool.Account1";
const char operationInterface[] = "org.rockpool.Operation1";
...
void PebbleAsyncTest::oauthUsesPrimaryOperationAndRefreshesOnlyAtTerminal()
{
    ...
    const QString operationPath = QStringLiteral("/org/rockpool/Operations/oauth_primary");
...
void PebbleAsyncTest::oauthOperationFailureIsExposed()
{
    ...
    account.deferSetOAuthToken();
    registerAccountService(&account, connection);
    const QString operationPath = QStringLiteral("/org/rockpool/Operations/oauth_failure");
    AccountTokenOperation operation(operationPath, connection);
    registerAccountOperation(&operation, operationPath, connection);
    ...
    operation.finish(false, QStringLiteral("org.rockpool.Error.IO"),
...
void PebbleAsyncTest::olderOAuthOperationIsRetainedButCannotFinishLatestWrite()
{
    ...
    account.deferSetOAuthToken();
    registerAccountService(&account, connection);
    const QString firstPath = QStringLiteral("/org/rockpool/Operations/oauth_older");
    const QString secondPath = QStringLiteral("/org/rockpool/Operations/oauth_latest");
    ...
    firstOperation.finish(false, QStringLiteral("org.rockpool.Error.IO"),
...
void PebbleAsyncTest::staleOAuthMethodReplyIsIgnored()
{
    ...
    account.deferSetOAuthToken();
    registerAccountService(&account, connection);
    const QString stalePath = QStringLiteral("/org/rockpool/Operations/oauth_stale_reply");
    const QString latestPath = QStringLiteral("/org/rockpool/Operations/oauth_current_reply");
...
{
    QDBusConnection rockworkConnection = QDBusConnection::connectToBus(
        QDBusConnection::SessionBus, QStringLiteral("pebble-async-oauth-owner-rockwork"));
    QVERIFY(rockworkConnection.isConnected());
    DelayedPebble watch(rockworkConnection);
    registerWatch(&watch, rockworkConnection);
    registerService(rockworkConnection);
    ...
    const QString oldOperationPath =
        QStringLiteral("/org/rockpool/Operations/oauth_old_owner_operation");
    const QString lateReplyPath =
        QStringLiteral("/org/rockpool/Operations/oauth_old_owner_reply");
    ...
    const QString newOperationPath =
        QStringLiteral("/org/rockpool/Operations/oauth_new_owner");
    ...
    QVERIFY(secondConnection.interface()->unregisterService(
                QString::fromLatin1(accountServiceName)).isValid());
    QVERIFY(rockworkConnection.interface()->unregisterService(
                QString::fromLatin1(serviceName)).isValid());
}
...
void PebbleAsyncTest::pebbleFacadesShareLatestOAuthOperationState()
{
    ...
    const QString olderPath =
        QStringLiteral("/org/rockpool/Operations/oauth_shared_older");
    const QString latestPath =
        QStringLiteral("/org/rockpool/Operations/oauth_shared_latest");
    ...
    olderOperation.finish(false, QStringLiteral("org.rockpool.Error.IO"),
    ...
    latestOperation.finish(false, QStringLiteral("org.rockpool.Error.IO"),
...
void PebbleAsyncTest::sharedOAuthOperationSurvivesPebbleReplacement()
{
    ...
    registerAccountService(&account, connection);
    const QString operationPath =
        QStringLiteral("/org/rockpool/Operations/oauth_facade_replacement");
    AccountTokenOperation operation(operationPath, connection);
    registerAccountOperation(&operation, operationPath, connection);
...
{
    QDBusConnection rockworkConnection = QDBusConnection::connectToBus(
        QDBusConnection::SessionBus, QStringLiteral("pebble-async-account-rockwork"));
    QVERIFY(rockworkConnection.isConnected());
    DelayedPebble watch(rockworkConnection);
    registerWatch(&watch, rockworkConnection);
    registerService(rockworkConnection);
    ...
    QVERIFY(secondAccountConnection.interface()->unregisterService(
                QString::fromLatin1(accountServiceName)).isValid());
    QVERIFY(rockworkConnection.interface()->unregisterService(QString::fromLatin1(serviceName)).isValid());
}
